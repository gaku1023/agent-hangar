...
`cloud.json` を読めた起動のうち、行が無い最初の 1 回だけ刻む（`sync/transcriptsFrom.ts`）。
`cloud.json` の `joinedAt` を使わないのは、あれが参加し直しと秘密の作り直しで今の時刻へ書き換わるからである。
区切りが後ろへ動くと、まだ上げ切っていない本文が取り残される。
既にクラウドと同期していた端末には区切りを置かず、0（区切りなし）を刻む。
