...
- 台帳（`meta` の `d1_rows:<yyyy-MM-dd>`）への書き込みも数に入れるが、毎回書くと見張り自身が枠を食う（1 要求 1 行しか書かない `GET /changes` では書き込みが 3 倍になる）。
  64 行たまってから次の batch の末尾に 1 文だけ混ぜるので、要求も往復も増えない。
  isolate が消えると、書き出す前の最大 64 行ぶんは落ちる。
...
- 諦めた本文とまだ上げていない本文の件数は `SyncDetailDto` に載せ、HTTP の応答だけで配る。
  websocket の `sync.status` を組み立てるのは `SyncEngine` で、どちらの値も持っていないからである。
  受け取った側は最後に見た値を覚えておき、ヘッダーと Settings に出す。
