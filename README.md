...
- セッションの本文（gzip して AES-256-GCM で暗号化。鍵は参加用の秘密から導くので、Cloudflare 側は中身を読めません。暗号化していないと申告された本文は、Worker が R2 に触る前に断ります）
...
本文（セッションの中身）は、クラウドを使い始めた後に動いたものだけを上げます。
参加より前に止まっている本文は上がりません。
新しく始めたセッションと、再開したセッションが対象です。
メタデータ（セッションの一覧、要約、プロジェクト、TODO、メモ）は今までどおり全部同期するので、他端末からも一覧と検索の結果は揃います。

参加より前の本文を上げたくなったら、そのセッションを再開してください。
ファイルが伸びた時点で、いつもの経路で上がります。
まとめて全部上げ直すときは、次を実行します。

```sh
npm run hangar -- cloud backfill       # 参加より前の本文も上げ直す
```
...
2. **無料枠の 80% に達すると同期が一時停止します。**
   D1 の書き込みは Worker が数えます。
   書き込みのある経路をすべて 1 つの関数に通し、D1 が返す `rows_written` をその日（UTC 区切り）の台帳に積んで、push の応答で返します。
   端末はその値を正として使うので、`changes` の圧縮、`join` の `devices` の upsert、cold start ごとの `ensureSchema` のように端末を通らない書き込みも入ります。
   報告はアカウント全体の数なので、端末の数では割りません。
   報告を受け取った後に自分で書いた分は、次の報告が来るまで端末側の見積もりで足します。
   報告を返さない古い Worker が相手のときと、要求の回数の側は、今までどおり端末の数で割った水準で見ます。
...
4. **`~/.agent-hangar/backups/` は直近 20 回ぶんだけ残します。**
   控えを取るのは、設定の取り込み（`claude-config/`）、「この PC で再開」での本文の上書き（`transcripts/`）、セッションのメモの競合（`memos/`）の 3 つで、どれも新しい方から 20 世代を残して古いものを消します。
   `claude-config/` は取り込みのたびに刈り、`transcripts/` と `memos/` は控えを取った後と、サーバを起こしたときに刈ります。
   この置き場の外に残る控え（プロジェクトのメモの隣の `memo.md.bak-<日時>` と、設定の同期が残す `*.conflict-*`）は消しません。
5. **孤児は掃除しますが、使わなくなった端末を畳む口はありません。**
   本文の PUT は R2、D1 の順、DELETE は索引を先に消す順なので、途中で倒れて残るのは索引に無い R2 の本体だけです。
   Worker は `GET /files` を契機に、6 時間に 1 回だけ R2 を 50 件と索引を 50 行まで見て、索引に無い本体と、本体の無い索引の行を消します。
   置いてから 1 時間たっていないものには触らないので、書いている最中の 1 本は消えません。
   誰も本文を降ろさない日は `GET /files` が来ないので走りませんが、孤児が増えるのも上げ下ろしをした日だけです。
   掃除自身が D1 に書くのは、孤児が数件のときの実測で 1 回 7 行、1 日 4 回で 28 行です（枠の 0.03%）。
   使わなくなった端末の `transcripts/<端末 ID>/` と `config/<端末 ID>/` を明示的に畳む操作は、まだありません。
...
8. **確認は 1 台の Mac で 2 端末を模して行いました。**
   `HANGAR_HOME` と `HANGAR_CLAUDE_DIR` を分け、2 つのポートで起こしています。
   実際に別のマシンから接続することは、まだ確かめていません。
   無料枠の数え方と孤児の掃除はこの確認より後に入れたもので、偽のクラウドとローカルの workerd（miniflare）の試験だけで確かめています。
   実物の Cloudflare では動かしていません。
9. **降ろすのを諦めた本文は画面に出ます。**
   ヘッダーに件数が出て、Settings のクラウド同期の節に鍵と理由と試した回数が並びます。
   同じ 2 か所に、まだ上げていない本文の件数も出ます。
