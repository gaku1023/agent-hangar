...
export function VirtualList<T>(props: { items: T[]; rowHeight: number | ((item: T, index: number) => number); height: number | string; render: (item: T, index: number) => ReactNode; keyOf: (item: T) => string; head?: ReactNode }) {
  const { items, rowHeight } = props;
  const parentRef = useRef<HTMLDivElement>(null);
  const v = useVirtualizer({ count: props.items.length, getScrollElement: () => parentRef.current, estimateSize: (i) => typeof rowHeight === 'function' ? rowHeight(items[i]!, i) : rowHeight, overscan: 12 });
